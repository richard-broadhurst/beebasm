...
void ObjectCode::PutByte( unsigned int byte )
{
	...
	{
		throw AsmException_AssembleError_Overlap(m_PC);
	}
...
*/
/*************************************************************************************************/
void ObjectCode::Assemble1( unsigned int opcode, LineParser * lp )
{
	if ( m_PC > 0xFFFF )
	{
		throw AsmException_AssembleError_OutOfMemory(lp);
	}
	...
	{
		throw AsmException_AssembleError_InconsistentCode(lp);
	}

	if ( m_aFlags[ m_PC ] & GUARD )
	{
		throw AsmException_AssembleError_GuardHit(lp);
	}

	if ( m_aFlags[ m_PC ] & USED )
	{
		throw AsmException_AssembleError_Overlap(lp, m_PC);
	}
...
*/
/*************************************************************************************************/
void ObjectCode::IncBin( const char* filename, LineParser * lp)
{
	ifstream binfile;
	...
	if ( !binfile )
	{
		throw AsmException_AssembleError_FileOpen(lp);
	}
	...
	while ( binfile.get( c ) )
	{
		...
		Assemble1( static_cast< unsigned char >( c ), lp );
	}

	if ( !binfile.eof() )
	{
		throw AsmException_AssembleError_FileRead(lp);
	}
